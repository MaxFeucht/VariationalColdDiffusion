...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        # Change Group Norm depending on whether out_channels is specified
        out_groups = num_groups if self.out_channels >= 32 else self.out_channels

        self.out_layers = nn.Sequential(
            normalization(self.out_channels, out_groups),
            nn.SiLU(),
...
class VAEEncoder(nn.Module):
    ...
    def __init__(
        ...
        # Build VAE output layers
        self.mean_layers = nn.ModuleList([])
        self.var_layers = nn.ModuleList([])

        # VAE mean and logvar channels added to the input
        # FORMERLY A VAE LAYER - ADD +2 TO THE OUT CHANNELS IF VAE IS ADDED
        ...
        # # Early Out Layer
        # self.early_out_layer = nn.Sequential(
        #     normalization(ch, 32),
        #     nn.SiLU(),
        #     conv_nd(dims, ch, 8, 3, padding=1, padding_mode=self.padding_mode),
        #     nn.Flatten()
        # )

        # self.mean_layers.append(nn.Sequential(self.early_out_layer,
        #                         nn.Linear(8*curr_res*curr_res, latent_dim)))
        # self.var_layers.append(nn.Sequential(self.early_out_layer,
        #                         nn.Linear(8*curr_res*curr_res, latent_dim)))
        ...
        for level, mult in enumerate(self.channel_mult):
            ...
            if level != len(self.channel_mult) - 1:
                ...
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ...
                        if self.resblock_updown
                            ...
                            ch, self.conv_resample, dims=dims, out_channels=out_ch + self.vae_ch, # +16 for mean and logvar
                            padding_mode=self.padding_mode
                        )
                    )
                )
                ...
                self.input_blocks.append(TimestepEmbedSequential(conv_nd(dims, ch, out_ch + self.vae_ch, 3, padding=1,
                                             padding_mode=self.padding_mode))
                )
            ...
            self.mean_layers.append(nn.Sequential(nn.Linear(ch*curr_res*curr_res, latent_dim), 
                                                            nn.SiLU(), 
                                                            nn.Linear(latent_dim, latent_dim)))
            self.var_layers.append(nn.Sequential(nn.Linear(ch*curr_res*curr_res, latent_dim),
                                                    nn.SiLU(), 
                                                    nn.Linear(latent_dim, latent_dim)))
    ...
    def forward(self, xt, cond, emb, priors=None, y=None):
        ...
        xt = th.cat([xt, cond], dim=1)

        h = xt.type(self.dtype)
        latents = []
        kls = []

        # Channel indices where VAE mean and logvar are outputted
        #vae_mults = [int(i * self.model_channels) + self.vae_ch for i in self.channel_mult]
        prev_res = self.image_size
        output_idx = 0
        ...
        for i, module in enumerate(self.input_blocks):
            ...
            # VAE Part
            # If resolution has changed or we are at the last layer
            if h.shape[-1] < prev_res or i == len(self.input_blocks) - 1: #or i == 0:

                mu = self.mean_layers[output_idx](h.flatten(1) if i != 0 else h)
                logvar = self.var_layers[output_idx](h.flatten(1) if i != 0 else h)

                z = th.randn_like(mu) * th.exp(0.5*logvar) + mu
...
class VAEUnet(nn.Module):
    ...
    def __init__(
        ...
        if self.vae_loc == 'emb' and self.vae_inject == 'concat':
            self.time_embed_dim = self.time_embed_dim * 2
        ...
        # self.vae_projections.append(nn.Linear(self.latent_dim, int(self.time_embed_dim/2)))
                ...
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ResBlock(
                            ...
                            self.time_embed_dim,
                            dropout,
                            out_channels=out_ch,  # +2 for mean and logvar
                            dims=self.dims,
                            use_checkpoint=use_checkpoint,
                            use_scale_shift_norm=self.use_scale_shift_norm,
                            down=True,
                        ...
                        if self.resblock_updown
                            ...
                            ch, self.conv_resample, dims=self.dims, out_channels=out_ch, # +2 for mean and logvar
                            padding_mode=self.padding_mode
                        )
                    )
                )
        ...
        for level, mult in list(enumerate(self.channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                layers = [
                    ResBlock(
                        ch + ich, # + mult * 2 for the z-sample that's repeated for mult amount of times
                        self.time_embed_dim,
                        dropout,
        ...
        print("Num input blocks: ", len(self.input_blocks))
        print("Num output blocks: ", len(self.output_blocks))
    ...
    def forward(self, xt, t, cond=None, prior=None, t2=None, y=None):
        ...
        # Obtain the VAE Latents
        if cond is not None:
            latents, kls = self.vae_encoder(xt, cond, emb, y=y)
        else:
            kls = [th.tensor(0).to(xt.device)]
            if prior is None:
                latents = th.randn(xt.shape[0], len(self.channel_mult), self.latent_dim).to(xt.device) # len(self.channel_mult) for the number of different latents, one for each level
            else:
                latents = prior.to(xt.device)
            
        
        # Reverse order of first dimension of latents for decoder
        enc_lat = latents
        dec_lat = latents.flip(1)
        ...
        if self.sample_noise is None:
            self.sample_noise = th.randn_like(xt).to(xt.device) 
        ...
        if self.add_noise: 
            if th.is_grad_enabled():
                ...
                xt = xt + self.sample_noise[:xt.shape[0]]
        ...
        for i, module in enumerate(self.input_blocks):
            ...
            temp_emb = emb
            z_emb = self.vae_projections[lat_idx](enc_lat[:,lat_idx,:])            
            #z_emb = enc_lat[:,lat_idx,:]
            temp_emb = th.cat([temp_emb, z_emb], dim=1)
            
            h = module(h, temp_emb)
            hs.append(h)
            
            # Take next latent if the resolution changes
            if h.shape[-1] != prev_res: #or i == 0:
                lat_idx += 1

            prev_res = h.shape[-1]


        h = self.middle_block(h, temp_emb)
        ...
        for j, module in enumerate(self.output_blocks):
            ...
            temp_emb = emb
            z_emb = self.vae_projections[-(lat_idx+1)](dec_lat[:,lat_idx,:])            
            #z_emb = dec_lat[:,lat_idx,:]
            temp_emb = th.cat([temp_emb, z_emb], dim=1)
            ...
            # # Take next latent if the resolution changes
            if h.shape[-1] != prev_res: #or j == len(self.output_blocks) - 1:
                lat_idx += 1
        ...
        # # Emb for final output Layer
        # temp_emb = emb
        # z_emb = self.vae_projections[-(lat_idx+1)](dec_lat[:,lat_idx,:])            
        # temp_emb = th.cat([temp_emb, z_emb], dim=1)

        self.kl_div = th.stack(kls).sum()
        self.kls = kls
        self.latents = latents

        h = h.type(xt.dtype)
        return self.out(h, temp_emb)
